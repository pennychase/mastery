defmodule Mastery.Boundary.TemplateValidator do
  ...
  def validate_raw(_raw), do: "must be a string"
  ...
  def validate_generator(_generator), do: {:error, "must be a string/list or string/function pair"}

  def validate_checker(checker) when is_function(checker, 2), do: :ok
  def validate_checker(_checker), do: {:error, "must be an artity 2 function"}
end
